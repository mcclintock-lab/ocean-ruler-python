...
def get_quarter_circles(input_image):
    gray_img = cv2.cvtColor(input_image, cv2.COLOR_BGR2GRAY)
    img = cv2.medianBlur(gray_img, 5)
    cimg = cv2.cvtColor(img,cv2.COLOR_GRAY2BGR)

    num_cols = len(input_image[0])
    minRadius = num_cols/50
    maxRadius = num_cols/20
    circles = cv2.HoughCircles(img,cv2.HOUGH_GRADIENT,1,len(input_image)/8, param1=100,param2=30,minRadius=minRadius,
                               maxRadius=maxRadius)
    ...
    cv2.imshow("HoughCirlces", input_image)
    cv2.waitKey()
...
def read_args():
    ...
    print('--->>>>> image name: {0}'.format(imageName))
...
def find_abalone_length(is_deployed, req):
    ...
    return execute(imageName, image_full, showResults, rulerWidth, out_file)


def execute(imageName, image_full, showResults, rulerWidth, out_file):
