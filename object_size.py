...
def sort_by_matching_shape(contours, template_contours):
    ...
    minRulerVal = 1000
    minAbaloneVal = 1000
    abaloneArea = cv2.contourArea(abalone_shape)
    rulerArea = cv2.contourArea(ruler_shape)
    print("temp abalone area: ", abaloneArea, "ruler area: ", rulerArea)

    for i, image_contour in enumerate(contours):
        ab_val = cv2.matchShapes(image_contour,abalone_shape,1,0.0)
        shape_area = cv2.contourArea(image_contour)

        print ("ab val: ", ab_val, " for dex: ", i, " with area ", shape_area)
...
retval, thresh1 = cv2.threshold(gray,mn,200,cv2.THRESH_BINARY)
