...
def execute(imageName, image_full, showResults, is_deployed):
    ...
    print_time("to get templates and scaled images")
    quarter_width = 0.955
    abalone_contour, orig_contours = contour_utils.get_abalone_contour(rescaled_image.copy(), small_abalone_template_contour)
    print_time("----------------- to get abalone")
    #quarter_contour = contour_utils.get_quarter_contour(rescaled_image.copy(), abalone_contour, quarter_template_contour)
    quarterCenterX, quarterCenterY, quarterRadius, matches = contour_utils.get_quarter_dimensions(rescaled_image.copy(), abalone_contour, quarter_template_contour, False)

    print_time("to get quarter: {}, {}, {}".format(quarterCenterX, quarterCenterY, quarterRadius))

    showText = showResults and not is_deployed
    flipDrawing = orig_rows/orig_cols > 1.2

    new_drawing = rescaled_image.copy()
    pixelsPerMetric, abaloneLength, left_point, right_point, left_ruler_point, right_ruler_point = drawing.draw_both_contours(new_drawing, 
        abalone_contour,showText, flipDrawing, quarterCenterX, quarterCenterY, quarterRadius)
